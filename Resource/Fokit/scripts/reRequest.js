...
[rewrite_local]
^https:\/\/m\.360\.cn url script-response-body https://raw.githubusercontent.com/axtyet/comet_scripts/main/Resource/Fokit/scripts/reRequest.js

[task_local]
7 7 7 7 7 https://raw.githubusercontent.com/axtyet/comet_scripts/main/Resource/Fokit/scripts/reRequest.js, tag=请求重放, img-url=https://github.com/FoKit/Scripts/blob/main/images/debug.png?raw=true, enabled=true

 */

const $ = new Env('请求重放');
const reRequest = $.getjson('reRequest') || {};
const _options = $.toObj(reRequest['options']) || {};
...
function getRequest() {
  try {
    debug($request);
    _options['url'] = $request.url;
    _options['method'] = $request.method;
    _options['headers'] = ObjectKeys2LowerCase($request.headers);
    if ($request?.body) {
      _options['body'] = $request.body;
    }
    $.setdata($.toStr(_options), '@reRequest.options');
    $.msg($.name, ``, `请求重放信息获取成功 🎉`);
  } catch (err) {
    $.logErr(err);
  }
